...
The tool hard-caps output at 3 results per call (both browse and discovery)
and ranks discovery by per-session BM25 over a cron-heavy corpus, so neither
shape can produce a full census. Read the store directly instead.
...
- Open read-only: `sqlite3.connect("file:PATH?mode=ro", uri=True)` (no write lock).
