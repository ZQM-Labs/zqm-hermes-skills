...
WHY: `session_search` (the tool) is NOT a census. Its BROWSE shape returns
only the 10 most-recent rows and its FTS5 DISCOVERY shape demotes
`source:"cron"`. Any count derived from it is wrong. The real session
store is a SQLite DB on disk. This script reads it read-only and prints a
true full index.
...
def main():
    ...
    print("\nNOTE: counts above are the AUTHORITATIVE census from state.db. "
          "Do NOT trust session_search browse/discovery for totals.")
